(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewCustomerSavingDetailsController: function (scope, routeParams, resourceFactory, paginatorService, location, $uibModal, route, dateFilter, $sce, $rootScope, API_VERSION) {
            scope.report = false;
            ...
            scope.routeTo = function (savingsAccountId, transactionId, accountTransfer, transferId) {
                if (accountTransfer) {
                    location.path('/viewcustomeraccounttransfers/' + transferId);
                } else {
                    location.path('/viewcustomersavingtrxn/' + savingsAccountId + '/trxnId/' + transactionId);
                }
            };
            ...
            scope.clickEvent = function (eventName, accountId) {
                ...
                switch (eventName) {
                    ...
                    case "activate":
                        ...
                        break;
                    case "deposit":
                        location.path('/customersavingaccount/' + accountId + '/deposit');
                        break;
                    case "withdraw":
                        location.path('/customersavingaccount/' + accountId + '/withdrawal');
                        break;
                    case "addcharge":
            ...
            scope.viewJournalEntries = function(){
                location.path("/customersearchtransaction/").search({savingsId: scope.savingaccountdetails.id});
            };
            ...
            scope.viewSavingsTransactionReceipts = function (transactionId) {
                ...
                scope.hidePentahoReport = true;
                scope.formData.outputType = 'PDF';
                scope.baseURL = $rootScope.hostUrl + API_VERSION + "/runreports/" + encodeURIComponent("Customer Savings Transaction Receipt");
                scope.baseURL += "?output-type=" + encodeURIComponent(scope.formData.outputType) + "&tenantIdentifier=" + $rootScope.tenantIdentifier+"&locale="+scope.optlang.code;
    ...
    mifosX.ng.application.controller('ViewCustomerSavingDetailsController', ['$scope', '$routeParams', 'ResourceFactory','PaginatorService' , '$location','$uibModal', '$route', 'dateFilter', '$sce', '$rootScope', 'API_VERSION', mifosX.controllers.ViewCustomerSavingDetailsController]).run(function ($log) {
        $log.info("ViewCustomerSavingDetailsController initialized");
    });
}(mifosX.controllers || {}));