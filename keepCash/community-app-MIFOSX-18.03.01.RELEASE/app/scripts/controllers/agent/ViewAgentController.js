           ...
           // scope.families=[];
            ...
            scope.routeToSaving = function (id, depositTypeCode) {
                if (depositTypeCode === "depositAccountType.savingsDeposit") {
                    location.path('/viewagentsavingaccount/' + id);
            ...
            scope.getEmployeesByAgent = function(){
                scope.EmployeesPerPage = 15;
                resourceFactory.agentEmployeeResource.getEmployeesUnderAgent({clientId: routeParams.id}, function (data) {
                    scope.employees = data;
                });
            };
            
            scope.employeeRouteTo = function (id) {
            ...
            scope.dataTableChange = function (clientdatatable) {
                resourceFactory.DataTablesResource.getTableDetails({datatablename: clientdatatable.registeredTableName,
                   ...
                   // scope.datatabledetails.columnHeaders.splice(0,1);
                   // data.data[0].row.splice(0,1);
                   
                    if(scope.datatabledetails.columnHeaders[0].columnName == "client_id"){
                        scope.datatabledetails.columnHeaders[0].columnName = "agent_id";
                    }
            ...
            scope.getAgentReceipts = function () {
                resourceFactory.clientReceiptsResource.getAllClientReceipt({clientId: routeParams.id}, function (data) {
                    for (var l in data) {
            ...
            scope.previewDocument = function (url, fileName) {
                scope.preview =  true;
                scope.previewRece =  false;
                scope.previewIdentities = false;
            ...
            scope.previewIdentitie = function (url, fileName) {
                scope.preview =  false;
                scope.previewRece =  false;
                scope.previewIdentities = true;
                scope.fileUrl = scope.hostUrl + url;
                if(fileName.toLowerCase().indexOf('.png') != -1)
                    scope.fileType = 'image/png';
                else if(fileName.toLowerCase().indexOf('.jpg') != -1)
            ...
            scope.previewReceipt = function (url, fileName) {
                scope.previewRece =  true;
                scope.preview =  false;
                scope.previewIdentities = false;
                scope.fileUrl = scope.hostUrl + url;
                if(fileName.toLowerCase().indexOf('.png') != -1)
