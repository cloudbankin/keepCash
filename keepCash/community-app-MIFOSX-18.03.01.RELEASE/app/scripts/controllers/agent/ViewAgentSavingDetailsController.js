(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewAgentSavingDetailsController: function (scope, routeParams, resourceFactory, paginatorService, location, $uibModal, route, dateFilter, $sce, $rootScope, API_VERSION) {
            scope.report = false;
            ...
            scope.routeTo = function (savingsAccountId, transactionId, accountTransfer, transferId) {
                if (accountTransfer) {
                    location.path('/viewagentaccounttransfers/' + transferId);
                } else {
                    location.path('/viewagentsavingtrxn/' + savingsAccountId + '/trxnId/' + transactionId);
                }
            };
            ...
            scope.clickEvent = function (eventName, accountId) {
                ...
                switch (eventName) {
                    ...
                    case "activate":
                        ...
                        break;
                    case "deposit":
                        location.path('/agentsavingaccount/' + accountId + '/deposit');
                        break;
                    case "withdraw":
                        location.path('/agentsavingaccount/' + accountId + '/withdrawal');
                        break;
                    case "addcharge":
            ...
            scope.viewJournalEntries = function(){
                location.path("/agentsearchtransaction/").search({savingsId: scope.savingaccountdetails.id});
            };
            ...
            scope.viewSavingsTransactionReceipts = function (transactionId) {
                ...
                scope.hidePentahoReport = true;
                scope.formData.outputType = 'PDF';
                scope.baseURL = $rootScope.hostUrl + API_VERSION + "/runreports/" + encodeURIComponent("Agent Savings Transaction Receipt");
                scope.baseURL += "?output-type=" + encodeURIComponent(scope.formData.outputType) + "&tenantIdentifier=" + $rootScope.tenantIdentifier+"&locale="+scope.optlang.code;
    ...
    mifosX.ng.application.controller('ViewAgentSavingDetailsController', ['$scope', '$routeParams', 'ResourceFactory','PaginatorService' , '$location','$uibModal', '$route', 'dateFilter', '$sce', '$rootScope', 'API_VERSION', mifosX.controllers.ViewAgentSavingDetailsController]).run(function ($log) {
        $log.info("ViewAgentSavingDetailsController initialized");
    });
}(mifosX.controllers || {}));